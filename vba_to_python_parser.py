...
def vba_to_openpyxl(vba_code: str) -> str:
    ...
    for line in vba_code.splitlines():
        ...
        # Sub / Function start
        m = re.search(r'\b(Sub|Function)\b\s+(\w+)', stripped, re.IGNORECASE)
        if m:
            arr_string = stripped.split(" ")
            for index, val in enumerate(arr_string):
                if re.search(r'\b(Sub|Function)\b', val, re.IGNORECASE):
                    func_name = arr_string[index+1]
        ...
        if re.search(r'^End\s+(Sub|Function)', stripped, re.IGNORECASE):
            python_lines.append("    pass  # End of function")
            continue

        # For loop
        m = re.search(r'^For\s+(\w+)\s*=\s*(\d+)\s+To\s+(\d+)', stripped, re.IGNORECASE)
        if m:
        ...
        # Next
        if re.search(r'^Next', stripped, re.IGNORECASE):
            continue

        # If statements
        m = re.search(r'^If\s+(.+)\s+Then', stripped, re.IGNORECASE)
        if m:
            cond = m.group(1)
            python_lines.append(f"    if {cond}:  # TODO: adjust condition syntax")
            continue

        m = re.search(r'^ElseIf\s+(.+)\s+Then', stripped, re.IGNORECASE)
        if m:
            cond = m.group(1)
            python_lines.append(f"    elif {cond}:  # TODO: adjust condition syntax")
            continue

        if re.search(r'^Else$', stripped, re.IGNORECASE):
            python_lines.append("    else:")
            continue

        if re.search(r'^End\s+If', stripped, re.IGNORECASE):
            continue

        # With ... End With
        m = re.search(r'^With\s+(.+)', stripped, re.IGNORECASE)
        if m:
            obj = m.group(1)
        ...
        if re.search(r'^End\s+With', stripped, re.IGNORECASE):
            if with_stack:
                with_stack.pop()
            continue

        # Select Case
        m = re.search(r'^Select\s+Case\s+(.+)', stripped, re.IGNORECASE)
        if m:
            var = m.group(1)
            python_lines.append(f"    search {var}:  # Python 3.10+ search/case")
            continue

        m = re.search(r'^Case\s+(.+)', stripped, re.IGNORECASE)
        if m:
            val = m.group(1)
            python_lines.append(f"        case {val}:")
            continue

        if re.search(r'^End\s+Select', stripped, re.IGNORECASE):
            continue

        # Do While / Loop Until
        m = re.search(r'^Do\s+While\s+(.+)', stripped, re.IGNORECASE)
        if m:
            cond = m.group(1)
            python_lines.append(f"    while {cond}:  # TODO: adjust condition syntax")
            continue

        m = re.search(r'^Loop\s+Until\s+(.+)', stripped, re.IGNORECASE)
        if m:
            cond = m.group(1)
            python_lines.append(f"    while not ({cond}):  # TODO: adjust condition syntax")
            continue

        if re.search(r'^Loop$', stripped, re.IGNORECASE):
            continue
...
def main():
    ...
    vba_code = Path("D:\\Python Projects\\vba_example.cls").read_text(encoding="utf-8", errors="ignore")
    python_code = vba_to_openpyxl(vba_code)
    Path("output.py").write_text(python_code, encoding="utf-8")
    # if args.output:
